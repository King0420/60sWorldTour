<?php
include("./data/60SReadWorld/function.php");
include("./data/60SReadWorld/cURL.php");
use YingApi\Curl;
header("content-type:image/jpeg");
$font=realpath('./data/60SReadWorld/13.ttf');
$date_font=realpath('./data/60SReadWorld/2.ttf');
$image_path='./data/60SReadWorld/60s_'.date('D').'.jpg';
list($width,$height)=getimagesize($image_path);
//获取高宽
$image=imagecreatefromjpeg($image_path);
$back=imagecolorallocate($image,00,00,00);
//日期
$box=imagettfbbox(20,0,$date_font,date("m")."月".date("d")."日");
imagettftext($image,20,0,855,610,$back,$date_font,date("Y")."年");
imagettftext($image,20,0,($width-$box[2]-38),640,$back,$date_font,date("m")."月".date("d")."日");
//农历
$today=new today();
$today=$today->convertSolarToLunar(date('Y'),date('m'),date('d'));
imagettftext($image,20,0,42,610,$back,$date_font,"农历");
imagettftext($image,20,0,42,640,$back,$date_font,$today[1].$today[2]);
//新闻
clearstatcache();//清除缓存
$new_path="./data/60SReadWorld/新闻.txt";
...
$news.=chunkSplit($new,27,"\n")."\n";
}
file_put_contents($new_path,$news);
}
imagettftext($image,25,0,45,720,$back,$font,$news);
//微语
$yiyan=Curl::get('https://tenapi.cn/yiyan')->body();
imagettftext($image,22,0,45,1840,$back,$font,"【微语】{$yiyan}");
//显示图片
imagejpeg($image);
//销毁图片
imagedestroy($image);
